...
""" MultiQC submodule to parse output from Sentieon HsMetricsAlgo """
...
def parse_reports(self):
    ...
    # Set up vars
    self.sentieon_HsMetrics_data = dict()

    # Go through logs and find Metrics
    for f in self.find_log_files("sentieon/hsmetrics", filehandles=True):
        parsed_data = dict()
        s_name = None
        keys = None
        commadecimal = None
        for l in f["f"]:
            # New log starting
            if s_name is None and "--algo HsMetricAlgo" in l:
                fn_search = re.search(r"-i(?:=|\s+)(\[?[^\s]+\]?)", l, flags=re.IGNORECASE)
                if fn_search:
            ...
            if s_name is not None:
                if "#SentieonCommandLine" in l and "--algo HsMetricAlgo" in l:
        ...
        for s_name in parsed_data.keys():
            for j in parsed_data[s_name].keys():
                ...
                if this_s_name in self.sentieon_HsMetrics_data:
                ...
                self.sentieon_HsMetrics_data[this_s_name] = parsed_data[s_name][j]

    # Filter to strip out ignored sample names
    self.sentieon_HsMetrics_data = self.ignore_samples(self.sentieon_HsMetrics_data)

    if len(self.sentieon_HsMetrics_data) > 0:

        # Write parsed data to a file
        self.write_data_file(self.sentieon_HsMetrics_data, "multiqc_sentieon_HsMetrics")

        # Add to general stats table
        # Swap question marks with -1
        data = self.sentieon_HsMetrics_data
        for s_name in data:
        ...
        try:
            covs = config.sentieon_config["general_stats_target_coverage"]
            assert type(covs) == list
            assert len(covs) > 0
            covs = [str(i) for i in covs]
            log.debug("Custom Sentieon coverage thresholds: {}".format(", ".join([i for i in covs])))
        ...
        if hs_pen_plot is not None:
            self.add_section(
                name="HS Penalty",
                anchor="sentieon_hsmetrics_hs_penalty",
    ...
    return len(self.sentieon_HsMetrics_data)


def _get_table_headers(data):
    # Look for a user config of which table columns we should use
    sentieon_config = getattr(config, "sentieon_config", {})
    HsMetrics_table_cols = sentieon_config.get("HsMetrics_table_cols")
    HsMetrics_table_cols_hidden = sentieon_config.get("HsMetrics_table_cols_hidden")
...
def _add_target_bases(data):
    ...
    pconfig = {
        "id": "sentieon_percentage_target_bases",
        "title": "Sentieon: Percentage of target bases",
        "xlab": "Fold Coverage",
        "ylab": "Pct of bases",
        "ymax": 100,
        "ymin": 0,
    ...
    return {
        ...
        "anchor": "sentieon_hsmetrics_target_bases",
...
def hs_penalty_plot(data):
    ...
    pconfig = {
        "id": "sentieon_hybrid_selection_penalty",
        "title": "Sentieon: Hybrid Selection Penalty",
        "xlab": "Fold Coverage",
        "ylab": "Penalty",
        "ymin": 0,
        "xmin": 0,
